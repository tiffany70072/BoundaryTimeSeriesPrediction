...
class GRUCell(Layer):
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            ...
            print('Eating: different implement setting?')
            exit()
            if 0. < self.dropout < 1.:
                inputs *= dp_mask[0]
        ...
        #h = z * h_tm1 + (1 - z) * hh # h_tm1: last time hidden state, hh: new state

        # new setting: set 1 - z to zero, if m = 1 (is_under)
        m_squeeze = tf.squeeze(m, 1)
        h = K.tf.where(tf.equal(m_squeeze, 1.), h_tm1, z * h_tm1 + (1 - z) * hh) # close: default equation (here!)
        #threshold = tf.fill(tf.shape(m_squeeze), 0.5)
        #h = K.tf.where(tf.greater(m_squeeze, threshold), h_tm1, z * h_tm1 + (1 - z) * hh)
        
        # clip z
        #z_clip = tf.clip_by_value(z, 0.5, 1.0) # z must be larger than 0.5
        #z = tf.where(tf.equal(m_squeeze, 1.), z_clip, z)
        #h = z * h_tm1 + (1 - z) * hh
