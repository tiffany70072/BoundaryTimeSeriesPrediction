    ...
    def build(self, input_shape):
        ...
        if self.use_bias:
            ...
            self.nan_bias = self.add_weight(shape=bias_shape,
                                        ...
                                        constraint=self.bias_constraint)
            if not self.reset_after:
                self.input_bias, self.input_nan_bias, self.recurrent_bias = self.bias, self.nan_bias, None
            else:
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            ...
            if self.use_bias:
                ...
                print(x_z.shape, self.input_bias_z.shape, m.shape, self.input_nan_bias_z.shape)
                # (?, 256) (256,) (?, 1) (256,)
                m_squeeze = tf.squeeze(m, 1) # m = 1 --> is_under
                #x_z = K.bias_add(x_z, self.input_bias_z) + K.bias_add(m_squeeze, self.input_nan_bias_z)
                #x_r = K.bias_add(x_r, self.input_bias_r) + K.bias_add(m_squeeze, self.input_nan_bias_r)
                #x_h = K.bias_add(x_h, self.input_bias_h) + K.bias_add(m_squeeze, self.input_nan_bias_h)
